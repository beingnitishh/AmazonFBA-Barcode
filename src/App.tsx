...
function LabelPreview({ row }: { row: LabelRow }) {
  return <svg viewBox="0 0 144 72" className="label-svg" role="img" aria-label={`Code 128 barcode for ${row.fnsku}, MRP ₹${row.mrpDisplay}`}>
...
export default function App() {
  ...
  const [fCol, setFCol] = useState(-1), [mCol, setMCol] = useState(-1);
  ...
  const rows = useMemo(() => sheet && mapped ? validateRows(sheet, fCol, mCol) : [], [sheet, mapped, fCol, mCol]);
  ...
  function reset() {
    setSheet(null); setMapped(false); setMapping(false); setFCol(-1); setMCol(-1); setPdf(null); setError(''); setPreview(0); setAcknowledged(false); setShowData(false); setDataPage(0);
    if (inputRef.current) inputRef.current.value = '';
  }
  function loadSheet(data: SheetData) {
    setSheet(data); setFCol(data.fnskuColumn); setMCol(data.mrpColumn);
    const detected = data.fnskuColumn >= 0 && data.mrpColumn >= 0;
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell">
      ...
      <main>
          ...
          <div className="left-column">
            <section className="card upload-card">
              ...
              {!sheet ? <div className={`dropzone ${drag ? 'dragging' : ''}`} onDragOver={e => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={e => { e.preventDefault(); setDrag(false); upload(e.dataTransfer.files[0]); }}>
                ...
                <div className="detected-columns"><div><span>FNSKU column</span><strong>{mapped ? <><Check size={14} />{sheet.headers[fCol]}</> : 'Selection required'}</strong></div><div><span>MRP column</span><strong>{mapped ? <><Check size={14} />{sheet.headers[mCol]}</> : 'Selection required'}</strong></div></div>
                ...
                {mapping && <div className="mapping-panel"><h3>Column mapping</h3><p>Select the correct columns. Missing or ambiguous headers need your confirmation.</p><div className="mapping-fields"><label>FNSKU<select value={fCol} onChange={e => setFCol(Number(e.target.value))}><option value={-1}>Select column</option>{sheet.headers.map((h, i) => <option key={i} value={i}>{h || `Column ${i + 1}`} ({i + 1})</option>)}</select></label><label>MRP<select value={mCol} onChange={e => setMCol(Number(e.target.value))}><option value={-1}>Select column</option>{sheet.headers.map((h, i) => <option key={i} value={i}>{h || `Column ${i + 1}`} ({i + 1})</option>)}</select></label></div><button className="button primary small" disabled={fCol < 0 || mCol < 0 || fCol === mCol} onClick={() => { setMapped(true); setMapping(false); setAcknowledged(false); setPreview(0); }}>Apply mapping <Check size={14} /></button></div>}
              </div>}
            ...
            {sheet && mapped ? <section className="card validation-card"><div className="section-header"><h2><ShieldCheck size={17} /> Data validation</h2><span className={`pill ${invalidRows.length ? 'red' : duplicateGroups.length ? 'amber' : 'green'}`}>{invalidRows.length ? 'Needs attention' : duplicateGroups.length ? 'Review duplicates' : 'Checks passed'}</span></div><div className="validation-stats"><div><strong>{rows.length}</strong><span>Total rows</span></div><div><strong className="green-text">{validRows.length}</strong><span>Valid rows</span></div><div><strong className={invalidRows.length ? 'red-text' : ''}>{invalidRows.length}</strong><span>Invalid rows</span></div><div><strong>{duplicateGroups.length}</strong><span>Duplicates</span></div></div>
              ...
              </section> : <section className="card requirements-card"><div className="section-header"><h2><FileSpreadsheet size={17} /> A simple sheet is all you need</h2><button className="text-button" onClick={downloadTemplate}><Download size={14} /> Get template</button></div><div className="requirements-content"><div className="requirements-copy"><p>Include these two columns in your first worksheet.</p><span><CircleCheck size={14} /> Headers are detected automatically</span><span><CircleCheck size={14} /> One row becomes one label</span></div><div className="mini-sheet"><div className="sheet-row sheet-head"><span /><span>A</span><span>B</span></div><div className="sheet-row"><span>1</span><strong>FNSKU</strong><strong>MRP (₹)</strong></div><div className="sheet-row"><span>2</span><span>X001ABC123</span><span>499</span></div><div className="sheet-row"><span>3</span><span>X001ABC124</span><span>799</span></div></div></div></section>}
          </div>
        ...
        {showData && sheet && <section className="card data-card"><div className="section-header"><h2><FileText size={17} /> Spreadsheet review</h2><span>Original Excel row order</span></div><div className="data-scroll"><table><thead><tr><th>Excel row</th><th>FNSKU</th><th>MRP</th><th>Status</th></tr></thead><tbody>{rows.slice(dataPage * 10, dataPage * 10 + 10).map(r => <tr key={r.rowNumber}><td>{r.rowNumber}</td><td className="mono">{r.fnsku || '—'}</td><td>₹{r.mrpDisplay}</td><td><span className={`pill ${r.errors.length ? 'red' : r.duplicate ? 'amber' : 'green'}`}>{r.errors.length ? 'Invalid' : r.duplicate ? 'Duplicate' : 'Valid'}</span></td></tr>)}</tbody></table></div><div className="table-pagination"><span>{dataPage * 10 + 1}–{Math.min(dataPage * 10 + 10, rows.length)} of {rows.length} rows</span><div><button className="icon-button" aria-label="Previous data page" disabled={!dataPage} onClick={() => setDataPage(p => p - 1)}><ChevronLeft size={17} /></button><button className="icon-button" aria-label="Next data page" disabled={(dataPage + 1) * 10 >= rows.length} onClick={() => setDataPage(p => p + 1)}><ChevronRight size={17} /></button></div></div></section>}
